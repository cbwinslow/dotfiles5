...
Summary:
    ...
    This script shows how to:
      ...
      * Create a multi-step, democratic-ish workflow:
...
Notes:
    * This is a starter / reference implementation. Swap models, tools,
      and memory backends as you wire it into your full stack.
    * Designed to be dropped into a `src/` directory of a CrewAI project
      or run standalone once dependencies are installed.
...
from crewai_tools import (
    SerperDevTool,         # web search / web_ops-ish
    ScrapeWebsiteTool,     # more detailed web extraction
    FileReadTool,
    FileWriterTool,
)
...
def build_default_llm() -> LLM:
    """Return a default LLM for all agents.

    This uses environment variables so you can swap providers without
    touching code:
        - OPENAI_API_KEY (or other provider keys if configured)
        - CREWAI_DEFAULT_MODEL (optional, e.g. "gpt-4o" or "gpt-4.1")
    """
...
# ---------------------------------------------------------------------------
# Tool Builders (mapping your logical tools to crewai-tools)
# ---------------------------------------------------------------------------


def build_web_research_tools():
    """Tools for the Web Research Agent.

    Rough mapping from your logical toolset:
        - web_operations  -> SerperDevTool + ScrapeWebsiteTool
        - file_operations -> FileReadTool/FileWriterTool
    """
...
def build_data_processing_tools():
    ...
    For now we reuse file tools so the agent can read/write intermediate
    artifacts. You can later replace this with custom pandas/SQL tools
    implemented via BaseTool.
    """
...
def build_consensus_tools():
    """Tools for the Consensus Coordinator.

    Typically just needs read access so it can inspect prior outputs
    (e.g., markdown, JSON files produced by other tasks).
    """
...
def build_self_healing_tools():
    """Tools for the Self-Healing Orchestrator.

    Stub for now: in a real deployment this would wrap monitoring APIs,
    log search, or status endpoints.
    """
...
# ---------------------------------------------------------------------------
# Agent Factories (CrewAI Agents that mirror your OpenAI agents)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Tasks: democratic-ish multi-agent flow
# ---------------------------------------------------------------------------
...
def build_tasks(
    ...
    """Create a 4-step workflow using your agents.

    1. research_task       -> Web Research Agent
    2. analysis_task       -> Data Processing Agent (context: research)
    3. consensus_task      -> Consensus Agent (context: research + analysis)
    4. self_healing_task   -> Self-Healing Agent (context: all prior)
    """
...
# ---------------------------------------------------------------------------
# Crew Builder & Entry Point
# ---------------------------------------------------------------------------


def build_crew() -> Crew:
    """Build a CrewAI Crew that mirrors your OpenAI-style multi-agent stack."""
...
def main(topic: str = "AI agent frameworks and CrewAI integration") -> None:
    """Kick off the crew for a given topic.

    Example:
        OPENAI_API_KEY=... python crewai_openai_agents_bridge.py
    """
    ...
    # `result` is a TaskOutput; `result.raw` usually holds the final string.
    logger.info("Crew run completed. Final output (truncated to 500 chars):\n%s", str(result)[:500])
    print("\n=== FINAL OUTPUT (raw) ===\n")
    try:
        # Newer CrewAI versions often expose .raw
